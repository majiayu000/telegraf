...
- **collection_offset**:
  ...
  at the same time by manually scheduling them in time. Must be non-negative
  and must not exceed the interval.
...
- **collection_offset**:
  ...
  [interval][]. Must be non-negative and must not exceed the interval. The
  value must be non-zero to override the agent setting.
